...
<div class ="wrapper">
	<div class = "container">
		...
		<div class = "context">
			...
			<div>
				<c:if test="${prev}">
					<span>[ <a href="/board?id=${boardId}&num=${startPageNum - 1}">이전</a> ]</span>
				</c:if>
				
				<c:forEach begin="${startPageNum}" end="${endPageNum}" var="num">
			  		<span>
						<a href="/board?id=${boardId}&num=${num}">${num}</a>
					</span>
				</c:forEach>
				
				<c:if test="${next}">
					<span>[ <a href="/board?id=${boardId}&num=${endPageNum + 1}">다음</a> ]</span>
				</c:if>
			</div>
			<div>
				<br><br>
